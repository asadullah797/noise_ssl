...
class LinearClassifier(nn.Module):
    ...
    def forward(self, features, labels=None, label_mask=None):
        ...
        if labels is not None:
            ...
            prediction_labels = prob.reshape(-1, prob.size(-1))
            prediction_labels = prediction_labels.argmax(dim=1, keepdim=True).squeeze()
            ground_labels = labels_with_ignore_index.reshape(-1)
            results = prediction_labels == ground_labels
            ground_labels2 = ground_labels.detach().cpu().tolist()
            ...
            for i in range(len(results)):
                if ground_labels2[i] != 0:
                    if results[i] == True:
                        corrects.append(ground_labels2[i])
                    else:
                        wrongs.append(ground_labels2[i])
            ...
            return loss, prob.detach().cpu(), correct.detach().cpu(), valid.detach().cpu(), corrects, wrongs, prediction_labels.detach().cpu().tolist(), ground_labels.detach().cpu().tolist()
