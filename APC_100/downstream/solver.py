...
class Downstream_Solver(Solver):
    ...
    def set_model(self, inference=False):
        ...
        if not inference and self.fine_tune:
            ...
            #print("not inference but training APC Downstream Task")
            self.optimizer = Adam(self.classifier.parameters(), lr=self.learning_rate, betas=(0.9, 0.999))
            #self.optimizer = SGD(self.classifier.parameters(), lr=self.learning_rate)
            ...
            for features, labels in tqdm(self.dataloader, desc="Iteration"):
                try:
                    ...
                    if self.model_type == 'linear':
                        ...
                        loss, _, correct, valid, _, _, _, _ = self.classifier(representations, labels, label_mask)
                    elif self.model_type == 'rnn':
                        # labels: (batch_size, )
...
class Downstream_Tester(Downstream_Solver):
    ...
    def exec(self):
            ...
            with torch.no_grad():
                try:
                    ...
                    if self.model_type == 'linear':
                        ...
                        # labels: (batch_size, seq_len)
                        loss, logits, correct, valid, corrects, wrongs, predictions_label, grounds_label = self.classifier(representations, labels, label_mask)
                        correct_tot += corrects
       	       	       	wrong_tot += wrongs
                        predictions += predictions_label
                        grounds += grounds_label
                    elif self.model_type == 'rnn':
                        # labels: (batch_size, )
        ...
        correct_tot = Counter(correct_tot)
        wrong_tot = Counter(wrong_tot)

        data_dict = pickle.load(open('data/libri_phone/phone2idx.pkl', 'rb'))
        correct_predictions = {}
        wrong_predictions = {}
        for key1, value1 in data_dict.items():
            for key2, value2 in correct_tot.items():
                if value1 == key2:
                    correct_predictions[key1] = value2
            for key3, value3 in wrong_tot.items():
                if value1 == key3:
                    wrong_predictions[key1] = value3

        correct_predictions = dict(collections.OrderedDict(sorted(correct_predictions.items())))
        wrong_predictions   = dict(collections.OrderedDict(sorted(wrong_predictions.items())))

        print('correct_predictions', correct_predictions)
        print('wrong_predictions', wrong_predictions)

        np.save('predictions.txt', np.array(predictions))
        np.save('grounds.txt', np.array(grounds))
