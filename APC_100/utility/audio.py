...
def extract_feature(input_file, feature='fbank', cmvn=True, save_feature=None):
    ...
    if feature == 'fbank':
        ...
        n_fft, hop_length, win_length = _stft_parameters(sr)
        feat1 = librosa.feature.melspectrogram(y=y, sr=sr, n_mels=num_mels,
                                              n_fft=n_fft, hop_length=hop_length, win_length=win_length)
        feat1 = librosa.power_to_db(feat1)
        feat2 = librosa.feature.mfcc(y=y, sr=sr, n_mfcc=13, n_mels=num_mels,
                                              n_fft=n_fft, hop_length=hop_length, win_length=win_length)

        feat2 = [feat2]
        feat2.append(librosa.feature.delta(feat2[0]))
        feat2.append(librosa.feature.delta(feat2[0], order=2))
        feat2 = np.concatenate(feat2, axis=0)
