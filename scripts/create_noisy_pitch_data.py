...
import augment
         ...
         #/home/people/22204840/scratch/data/data/LibriSpeech
         #/home/people/22204840/scratch/data/data/LibriSpeech
source = "/home/people/22204840/scratch/data/data/LibriSpeech/train-clean-100"
dest   = "/home/people/22204840/scratch/data/data/LibriSpeech/train_clean25_aug125_p125"
source = dest
sampling_rate=16000
    ...
    for wav_file in os.listdir(os.path.join(source, readers, chapters)):
      ...
      if wav_file.endswith('.flac'):
          ...
          if counter == 0:
              ...
              i=i+1       
              new_wav_file= wav_file.split('/')[-1].split('.')[0]+"-aug"+".flac"
              write(os.path.join(dest, readers, chapters, new_wav_file), sampling_rate, y) 
              counter = 1
          else:
              x, sr = torchaudio.load(os.path.join(source, readers, chapters, wav_file))
              y = augment.EffectChain().pitch(-200).rate(sr) \
              .apply(x, src_info={'rate': sr})
              new_wav_file= wav_file.split('/')[-1].split('.')[0]+"-pitch"+".flac"
              torchaudio.save(os.path.join(dest, readers, chapters, new_wav_file), y, sr)
              counter = 0              
          #shutil.copy(os.path.join(source, readers, chapters, wav_file), os.path.join(dest, readers, chapters), follow_symlinks=False)
      #shutil.copy(os.path.join(source, readers, chapters, readers+"-"+chapters+".trans.txt"), os.path.join(dest, readers, chapters), follow_symlinks=False)
