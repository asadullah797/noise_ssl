...
dest   = "/home/people/22204840/scratch/data/data/LibriSpeech/train_clean25_aug375_p375"
source = dest
sampling_rate=16000
  ...
  for chapters in os.listdir(os.path.join(source,readers)):
    for wav_file in os.listdir(os.path.join(source, readers, chapters)):
      ...
      if wav_file.endswith('.flac'):
          ...
          new_wav_file= wav_file.split('/')[-1].split('.')[0]+"-pitch1"+".flac"
          ...
          if counter == 0:
              ...
              y = augment.EffectChain().pitch(200).rate(sr) \
              .apply(x, src_info={'rate': sr})
              new_wav_file= wav_file.split('/')[-1].split('.')[0]+"-pitch2"+".flac"
              torchaudio.save(os.path.join(dest, readers, chapters, new_wav_file), y, sr)
              counter = 0
