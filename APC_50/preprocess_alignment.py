...
def phone_preprocess(data_path, output_path, sets, unaligned):
    ...
    for s in sets:
        ...
        paths = list(Path(os.path.join(data_path, s)).rglob("*.txt"))
        for path in tqdm(paths):
            ...
            if check_name not in unaligned and check_name != 'unaligned': # ignore the unaligned files and `unaligned.txt` itself
                for line in open(path).readlines():
                    phone = line.strip('\n').split(' ')[-1]
                    phone = phone.translate(remove_digits)
    ...
    for s in sets:
        ...
        for path in tqdm(todo):
            ...
            if check_name not in unaligned and check_name != 'unaligned': # ignore the unaligned files and `unaligned.txt` itself
                ...
                for line in file:
                    line = line.strip('\n').split(' ')
                    #phone = phone_map(line[2].translate(remove_digits))
                    x += time_to_frame(start_time=float(line[0]), end_time=float(line[1]), phone=phone2idx[line[2].translate(remove_digits)])
                x = np.asarray(x)
...
def main():
    ...
    # dump unaligned text
    try:
        file = open(os.path.join(args.data_path, 'train-clean-10/unaligned.txt')).readlines()
    ...
    # Process data
    #sets = ['train-clean-10'] # only two sets available for now
    #sets = ['train-clean-10','dev-clean','dev-other','test-clean','test-other', 'test-clean-aug']
