...
def subword_preprocess(args, sets, encode_table, output_dir, dim):
    ...
    for s in bpe_tr:
        ...
        tr_txt+=Parallel(n_jobs=args.n_jobs)(delayed(read_text)(str(file),target=args.target) for file in todo[:len(todo)//36])
    ...
    for s in sets:
        todo = list(Path(os.path.join(args.data_path,s)).rglob("*.flac"))
        txts = Parallel(n_jobs=args.n_jobs)(delayed(read_text)(str(file),target=args.target) for file in todo[:len(todo)//36])
...
def acoustic_preprocess(args, tr_set, encode_table, output_dir, bpe_dir, dim):
    ...
    for s in tr_set:
        ...
        todo = list(Path(os.path.join(args.data_path,s)).rglob("*.flac"))
        print(len(todo)//36,'audio files found in',s)
        ...
        if args.target == 'subword':
            ...
            tr_y = Parallel(n_jobs=args.n_jobs)(delayed(read_text)(str(file),target=args.target) for file in tqdm(todo[:len(todo)//36]))
        ...
        tr_x = Parallel(n_jobs=args.n_jobs)(delayed(extract_feature)(str(file), feature=args.feature_type, cmvn=args.apply_cmvn, \
                                            save_feature=os.path.join(cur_path, str(file).split('/')[-1].replace('.flac',''))) for file in tqdm(todo[:len(todo)//36]))
