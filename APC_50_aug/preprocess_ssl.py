...
def get_preprocess_args():
    ...
    parser.add_argument('--feature_type', default='mel', type=str, help='Feature type ( mfcc / fbank / mel / linear / mix)', required=False)
    ...
    parser.add_argument('--n_tokens', default=500, type=int, help='Vocabulary size of target', required=False)
...
def read_text(file,target):
    ...
    src_file = '-'.join(file.split('-')[0:5])+'.trans-comb.txt'
    idx = file.split('/')[-1].split('.')[0]

    #idx = os.path.splitext(file.split('/')[-1])[0]
...
def subword_preprocess(args, sets, encode_table, output_dir, dim):
    ...
    for s in bpe_tr:
        ...
        tr_txt+=Parallel(n_jobs=args.n_jobs)(delayed(read_text)(str(file),target=args.target) for file in todo[:len(todo)//2])
    ...
    spm.SentencePieceTrainer.train(input=os.path.join(bpe_dir,'train.txt'), 
                                    model_prefix=os.path.join(bpe_dir,'bpe'), 
    ...
    for s in sets:
        todo = list(Path(os.path.join(args.data_path,s)).rglob("*.flac"))
        txts = Parallel(n_jobs=args.n_jobs)(delayed(read_text)(str(file),target=args.target) for file in todo[:len(todo)//2])
        ...
        sp = spm.SentencePieceProcessor(model_file=os.path.join(bpe_dir,'bpe.model'))
        with open(os.path.join(bpe_dir,'raw',s+'.txt'), 'r') as rf, open(os.path.join(bpe_dir,'encode',s+'.txt'), 'w') as wf:
            for line in rf:
...
def acoustic_preprocess(args, tr_set, encode_table, output_dir, bpe_dir, dim):
    ...
    for s in tr_set:
        ...
        todo = list(Path(os.path.join(args.data_path,s)).rglob("*.flac"))
        print(len(todo)//2,'audio files found in',s)
        ...
        if args.target == 'subword':
            ...
            tr_y = Parallel(n_jobs=args.n_jobs)(delayed(read_text)(str(file),target=args.target) for file in tqdm(todo[:len(todo)//2]))
        ...
        tr_x = Parallel(n_jobs=args.n_jobs)(delayed(extract_feature)(str(file), feature=args.feature_type, cmvn=args.apply_cmvn, \
                                            save_feature=os.path.join(cur_path, str(file).split('/')[-1].replace('.flac',''))) for file in tqdm(todo[:len(todo)//2]))
...
def main():
    ...
    # Process data
    #sets = ['train-clean-100', 'train-clean-36', 'test-clean', 'test-other']
    sets = ['train-clean-50-aug']
